...
const MaxBrushSize = GetRandomFloat(0.1, 1.5)//GetRandomFloat(0.1, 2)
...
class KElement implements ISimObject {
	...
	computeImageBasedDrawing(imgData:ImageData){
		...
		const luma = this.getLuma(r, g, b)
		const ln = 1 - luma / 255.0 // normalise to (0,1)

		this.alpha = ln * (this.duration - this.ttl)/this.duration

		// compute change
		const dVelocity = lerp(-0.1, .1, ln)
		const dTheta = lerp(-Math.PI/8, Math.PI/8, ln) 
		...
		this.brushSize = lerp(0.1, MaxBrushSize, ln)
	}
...
function bootstrapper(){
	...
	// image.src='sym6.jpg'
	// image.src = 'church2.jpg'
	// image.src='gpe.jpg'
	// image.src = 'vg.jpg'
	// image.src = 'joker.jpg'
	// image.src = 'hokusai.jpg'
	// image.src = 'eiffel.jpg'
	// image.src = 'colloseum.jpg'
	image.src = 'elon.jpg'
}
