...
function GetSafeDelta(min: number, max: number, value: number, change: number): number {
	const upper = (value + change) > max
	const lower = (value + change) < min
	return (upper || lower) ? -change : change
}
function Clamp(min: number, max: number, value: number): number {
	return value > max ? max : (value < min ? min : value)
}
function lerp(a: number, b: number, scale: number) {
...
////////////////////////////////////////////////////////////////////////////////
class KElement implements ISimObject {
	x = 0; y = 0;
	x_ = 0; y_ = 0;
	v = 0; theta = 0; // velocity and direction
	brushSize = 1.0
	color = 'black'
	ttl = 500
	duration = 500
	...
	noiseScalar = [GetRandomFloat(0.000001, 0.000001), GetRandomFloat(0.0002, 0.004)]
	...
	reset() {
		...
		this.v = GetRandomFloat(1.0, 3.0)
		this.theta = GetRandomFloat(0, 2 * Math.PI)

		// aesthetics
		this.brushSize = GetRandomFloat(0.05, MaxBrushSize)
		this.color = 'black'
		if (Math.random() > 0.2) {
			this.color = this.palette[GetRandomInt(0, 6)]
		}
		this.alpha = 1

		// time to live
		this.ttl = GetRandomInt(25, 50)
		this.duration = this.ttl
	}

	getRandomLocation() {
		// const a = Math.min(this.w, this.h)
		// const scale = GetRandomFloat(0, a * 0.5)
		// const radius = Math.random() * 2 * Math.PI
		// const x = Math.cos(radius) * scale
		// const y = Math.sin(radius) * scale
		// return [ x + this.w/2 , y + this.h/2]

		return [GetRandomFloat(0, this.w), GetRandomFloat(0, this.h)]
	}

	getLuma(r: number, g: number, b: number): number {
		return 0.375 * r + 0.5 * g + 0.125 * b
	}

	computeImageBasedDrawing(imgData: ImageData) {
		const p = Math.floor(this.x) + Math.floor(this.y) * imgData.width
		// ImageData contains RGBA values, so we multiply by 4 to get to the correct
		// location
		...
		let luma = this.getLuma(r, g, b)
		const ln = 1 - luma / 255.0 // normalise to (0,1)

		this.alpha = (this.duration - this.ttl) / this.duration

		// compute change
		const dVelocity = GetRandomFloat(-0.3, 0.3) * ln //lerp(-0.1, .1, ln)
		const dTheta = GetRandomFloat(-Math.PI / 8, Math.PI / 8) * ln //lerp(-Math.PI / 8, Math.PI / 8, ln)

		this.v = this.v + dVelocity
		this.theta = this.theta + dTheta

		const [dx, dy] = FromPolar(this.v, this.theta)

		// preserve old position
		this.x_ = this.x; this.y_ = this.y;

		// compute new values
		this.x += GetSafeDelta(0, this.w, this.x, dx)
		this.y += GetSafeDelta(0, this.h, this.y, dy)
		this.x = Clamp(0, this.w - 1, this.x)
		this.y = Clamp(0, this.h - 1, this.y)
		this.brushSize = GetRandomFloat(0.1, MaxBrushSize) * ln //GetRandomFloat(0.1, MaxBrushSize)
	}

	computeFreeDrawing() {
		// compute change
		let dBrushSize = GetRandomFloat(-MaxBrushDelta, MaxBrushDelta)
		dBrushSize = GetSafeDelta(0.0, MaxBrushSize, this.brushSize, dBrushSize)

		const dVelocity = GetRandomFloat(-0.01, 0.01)
		const dTheta = GetRandomFloat(-Math.PI / 8.0, Math.PI / 8.0)
		this.v = this.v + dVelocity
		this.theta = this.theta + dTheta
		...
		this.x = this.x + dx
		this.y = this.y + dy
		this.brushSize = this.brushSize + dBrushSize
	}

	Update(imgData: ImageData) {
		this.computeImageBasedDrawing(imgData)
		// evaluate lifetime
		this.ttl -= 1
		if (this.ttl <= 0) {
			this.reset()
	...
	experiment1(ctx: CanvasRenderingContext2D) {
		...
		circle.arc(this.x, this.y, this.brushSize, 0, 2 * Math.PI);
		ctx.fill(circle);
	}
...
class Simulation implements ISimObject {
	...
	constructor(private width: number, private heigh: number) {
		for (let i = 0; i < ElementCount; i++) {
			...
			this.elements.push(
				new KElement(this.width, this.heigh, this.palette)
			)
		}
	}
	...
	Draw(ctx: CanvasRenderingContext2D) {
		...
		if (!this.init) {
			...
			ctx.fillRect(0, 0, this.width, this.heigh)
			this.init = true
		}
...
function createDrawCanvas(imageCtx: CanvasRenderingContext2D, width: number, height: number) {
	const updateFrameRate = 100
	const renderingFrameRate = 100
...
function bootstrapper() {
	...
	image.onload = (ev) => {
		...
		ctx1.drawImage(image, 0, 0, 400, 400);
		// create the painter
		createDrawCanvas(ctx1, width, height)
	}

	const imgsrc = ['gpe.jpg', 'vg.jpg', 'joker.jpg', 'hokusai.jpg', 'eiffel.jpg', 'colloseum.jpg', 'elon.jpg', 'scarjo.jpg']
	// set the image source
	image.src = imgsrc[GetRandomInt(0, imgsrc.length)]
